...
def _poll_job_until_complete(context, job_id: str, job_type: str, agent_config_data: Dict[str, Any], max_polls: int = MAX_POLLS):
    """
    ...
    while poll_count < max_polls and not job_completed:
        ...
        if poll_count > 0:  # Skip initial wait
            logging.info(f"[DURABLE-ORCHESTRATOR] Waiting {POLL_INTERVAL_MINUTES} minutes before next {job_type} poll (poll {poll_count+1}/{max_polls})")
            yield context.create_timer(context.current_utc_datetime + timedelta(minutes=POLL_INTERVAL_MINUTES))
        
        # Check job status (1 second)
        status_result = yield context.call_activity("check_job_status", {
            "job_id": job_id,
...
def _execute_research_jobs(context, research_topics: List[str], user_input: Dict[str, Any], 
                          agent_config_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for i, topic in enumerate(research_topics):
        ...
        job_info = yield context.call_activity("start_research_job", {
        ...
        if job_completed:
            result_data = yield context.call_activity("fetch_job_result", {
...
def _execute_synthesis_job(context, research_results: List[Dict[str, Any]], user_input: Dict[str, Any], 
                          agent_config_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    synthesis_job_info = yield context.call_activity("start_synthesis_job", {
...
def orchestrator_function(context: durablefunctions.DurableOrchestrationContext):
    ...
    try:
        ...
        required_fields = ["job_id", "user_input", "budget_tier", "spreadsheet_id", "research_plan"]
        ...
        yield context.call_activity("update_spreadsheet", {
            "job_id": job_id,
